...
function FluidMeter() {
  ...
  function drawFluidLayer(layer, dt) {
    ...
    var meterBottom = (options.size - (options.size - getMeterRadius()) / 2) - options.borderWidth;
    var fluidAmount = currentFillPercentage * (getMeterRadius() - options.borderWidth * 2) / options.valmaxi;
  ...
  return {
    init: function (env) {
      ...
      fillPercentage = clamp(env.fillPercentage, 0, options.valmaxi);

      if (env.options) {
        options.valmaxi = env.options.valmaxi;  
    ...
    setPercentage(percentage) {
      ...
      fillPercentage = clamp(percentage, 0, valmaxiafficher);
    }
  }
};